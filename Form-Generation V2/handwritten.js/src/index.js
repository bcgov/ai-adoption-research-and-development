...
function processText(rawText, lineWidthOption) {
  const batchSize = typeof lineWidthOption === "number" ? lineWidthOption : getBatchSize();
  const str = cleanText(
    rawText
      .split("\t")
      .join("     ")
...
async function main(rawText = "", optionalArgs = {}) {
  ...
  const lineWidth = optionalArgs.lineWidth != null ? optionalArgs.lineWidth : undefined;
  const [str, width] = processText(rawText, lineWidth);
  if (outputType === "pdf") {
